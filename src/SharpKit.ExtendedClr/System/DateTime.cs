...
")]
        ...
        [JsMethod(Name = "TryParse$$String$$DateTime")]
        public static bool TryParse(string str, out DateTime value)
        {
            JsNumber innerValue = JsDate.parse(str);
            if(innerValue == JsNumber.NaN)
            {
                value = new DateTime(1970, 1, 1);
                return false;
            }

            value = new DateTime(innerValue);
            return true;
        }
        ...
        [JsMethod(Code = "return Neptuo.DateFormat(this.date, format);")]
